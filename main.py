...
parser.add_argument('--initial-learning_rate', type=float, default=0.00001, metavar='N',
                    help='num of training epochs (default: 0.00001)')
...
V= np.array([mesh['V'] for mesh in data]).mean(0)
np.save('mean_shape',V)
F=np.load('faces.npy')
V, L, L_norm, Di, DiA, simple_Di = process([V, F])
np.save('mean_L',L)
np.save('mean_L_norm',L_norm)
np.save('mean_Di',Di)
...
for path in path_list:
    for i, sample in enumerate(np.load(path),allow_pickle=True):
